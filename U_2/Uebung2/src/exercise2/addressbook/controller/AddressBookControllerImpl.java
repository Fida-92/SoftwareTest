...
/**
 * Controller class for the address book example. Bootstraps and manages the
...
public class AddressBookControllerImpl implements AddressBookController {
    ...
    /**
     * Ctor. Creates components for the application and establishes internal
	 ...
	 * 
	 * @see
	 * exercise2.addressbook.controller.AddressBookController#add(java.lang.String,
	 * java.lang.String, java.lang.String, java.lang.String, java.lang.String)
     */
    @Override
    public void add(String firstName, String lastName, String gender, String phoneContactInformation,
            String emailContactInformation) throws ParameterException, SizeLimitReachedException {

        // Check parameters
        if (firstName == null) {
            throw new ParameterException("No first name");
        }
        if (lastName == null) {
            throw new ParameterException("No last name");
        }
        if (!(gender.equalsIgnoreCase("M") || gender.equalsIgnoreCase("F"))) {
            throw new ParameterException("Only \"M\" and \"F\" allowed as gender");
        }
        if ((phoneContactInformation != null) && (emailContactInformation != null)) {
            throw new ParameterException("Only one sort of contact information can be set at one time");
        }
        ...
        if (phoneContactInformation != null) {
            entry.setContactPhoneNumber(Integer.parseInt(phoneContactInformation));
        }
        if (emailContactInformation != null) {
            entry.setContactEmailAddress(emailContactInformation);
        }
        try {
            // Add entry
            ...
            // an Exception of Type SizeLimitReachedException muss be thrown here
            throw new SizeLimitReachedException("Couldn't add! Size limitation is ");
    ...
    public void erase() {
        ...
        while (entryIt.hasNext()) {
            ...
            if (this.model.getEntries().contains(entry)) {
                entryIt.remove();
            }
            // In diesem Fall wird die ConcurrentModificationException
            // vom der Methode this.model.deleteEntry(entryIt.next()) ausgelöst
            // da sowohl das lesen als auch löschen auf dasseble Collection angewendet wird;
        }
        this.view.refresh();
    }
}
